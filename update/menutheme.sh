...
###########- COLOR CODE -##############
colornow=$(cat /etc/hokagevpnvpn/theme/color.conf)
NC="\e[0m"
RED="\033[0;31m" 
COLOR1="$(cat /etc/hokagevpnvpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
COLBG1="$(cat /etc/hokagevpnvpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"                    
###########- END COLOR CODE -##########



BURIQ () {
    curl -sS https://raw.githubusercontent.com/hokagevpnlegend2023/ipmini/main/ijin > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/hokagevpnlegend2023/ipmini/main/ijin | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/hokagevpnlegend2023/ipmini/main/ijin | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
